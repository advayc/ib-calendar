...
const ClubFilter: React.FC<ClubFilterProps> = ({ activeDate, onChangeDate, theme = 'light' }) => {
  ...
  {/* Section header */}
  <div className={`px-4 text-[12px] tracking-wide font-semibold uppercase mt-2 mb-4 select-none flex-shrink-0 ${isLight ? 'text-gray-500' : 'text-gray-400'}`}>Clubs</div>
  <div className={`flex-1 overflow-y-auto px-2 pb-4 ${isLight ? 'bg-gray-50' : 'bg-[#101215]'}`}>
        {clubs.length === 0 ? (
          <div className={`px-4 py-2 text-sm ${isLight ? 'text-gray-500' : 'text-gray-400'}`}>
            No clubs available
          </div>
        ) : (
          clubs.map(club => (
